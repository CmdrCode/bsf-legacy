...
# account it was authored on. tools/bsf/paths.py answers this for the whole
# repo — $BSF_GAME, then $BSF_BASE, then a search near the checkout — and
# nothing in it raises at import time. Same resolution as tools/editor/assets.py
# by construction, since both read the one module rather than each other.
sys.path.insert(0, os.path.join(HERE, '..', '..', 'tools', 'bsf'))
...
DEFAULT_DIRS = [
    os.environ.get('BSF_GAME_DIR', ''),
    INSTALL,
    os.path.join(HERE, '..', '..', 'battleshipsforeverv090d'),
]
...
class Library:
    ...
    def from_game(cls, game_dir=None):
        import gm7
        exe = None
        for d in ([game_dir] if game_dir else DEFAULT_DIRS):
            if not d:
                continue
            p = os.path.join(os.path.expanduser(d), 'BattleshipsForever.exe')
            if os.path.exists(p):
                exe = p
                break
        if not exe:
            raise FileNotFoundError('BattleshipsForever.exe not found — set BSF_GAME_DIR')
    ...
    def manifest(self, names=None):
        ...
        for nm, s in self.sprites.items():
            if names and nm not in names:
                continue
            out[nm] = {'w': s['w'], 'h': s['h'], 'ox': s['ox'], 'oy': s['oy'],
                       'n': s['n'], 'mask': s['mask']}
        return out
